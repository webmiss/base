...
export default {
  ...
  methods:{
      ...
      if(this.wheel.length==0){
        ...
        setTimeout(()=>{
          ...
          for(let i=0; i<this.pickerData.length; i++){
            this.wheel.push(new BScroll(this.$refs.wheelWrapper.children[i], {
              probeType: 3,
              wheel: {
            ...
            this.wheel[i].on('scrollEnd', ()=>{
              const newIndex = this.wheel.map(wheel=>wheel.getSelectedIndex());
    ...
    // 菜单数据
    _loadPickerData(newIndex,oldIndex){
      let data = [];
      let menu = [];
      if(!oldIndex){
        // 重置
        ...
        if(this.pickerData.length>1 && newIndex[0]!==oldIndex[0]){
          data = this.data[newIndex[0]].children;
          if(data){
            menu = data.map(({label,value}) => ({label,value}));
