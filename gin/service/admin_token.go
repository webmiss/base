...
func (AdminToken) Perm(token string) map[string]int64 {
	...
	env := config.Env()
	redis := (&library.Redis{}).New("")
	permStr := redis.Get(env.AdminTokenPrefix + "_perm_" + util.Strval(tData["uid"]))
	redis.Close()
	// 拆分
	arr := []string{}
...
func (AdminToken) Create(data map[string]interface{}) string {
	...
	env := config.Env()
	redis := (&library.Redis{}).New("")
	key := env.AdminTokenPrefix + "_token_" + util.Strval(data["uid"])
	redis.Set(key, "1")
...
func (AdminToken) Token(token string) jwt.MapClaims {
	...
	if tData != nil {
		env := config.Env()
		redis := (&library.Redis{}).New("")
		tData["time"] = redis.TTL(env.AdminTokenPrefix + "_token_" + util.Strval(tData["uid"]))
		redis.Close()
	}
	return tData
}
