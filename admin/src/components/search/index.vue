    ...
    <div class="wm-search_body" v-if="checked">
      ...
      <ul class="wm-search_list scrollbar">
        <template v-if="dataList.length>0">
          <li v-for="(v,k) in dataList" :key="k" @click="selectClick(v.value)">{{v.label}}</li>
        </template>
...
.wm-search_input input:hover{border-color: #C2C4C6;}
.wm-search_input input:focus{outline: none; border-color: #6FB737;}
...
.wm-search_list li{cursor: pointer; line-height: 32px; padding: 0 16px;}
.wm-search_list li:hover{background-color: #F5F7FA; color: #595;}
...
export default defineComponent({
  ...
  props: {
    ...
    noneText: {type:String, default:'暂无结果'},      //提示信息
  },
  watch:{
    data(val: any){
      this.dataList = val;
