    ...
    <div class="wm-select_body" v-if="checked">
      ...
      <ul class="wm-select_list scrollbar" :style="{maxHeight: maxHeight}">
        <template v-for="(v,k) in dataList" :key="k">
          <li v-if="v.disabled" class="wm-select_disabled" style="cursor: not-allowed;">{{v.label}}</li>
          <li v-else :class="v.value==value?'wm-select_active':''" @click="selectClick(v.value)">{{v.label}}</li>
        </template>
      </ul>
    </div>
  </div>
...
.wm-select_input input{cursor: pointer; width: 100%; height: 100%; padding: 0 32px 0 16px; display: flex; box-sizing: border-box; border-radius: 4px; border: @BorderColor 1px solid; background-color: #FFF;}
...
.wm-select_body{position: absolute; z-index: 9999; width: 100%; margin-top: 10px; box-sizing: border-box; border: @BorderColor 1px solid; border-radius: 4px; background-color: #FFF; box-shadow: 0 0 12px rgba(0,0,0,.2);}
.wm-select_arrow{position: absolute; top: -15px; left: 50%; transform: translate(-50%, 0); width: 0px; height: 0px; border: 8px solid; border-color: transparent; border-bottom-color: #FFF;}
